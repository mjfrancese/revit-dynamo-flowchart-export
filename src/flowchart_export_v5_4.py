...
# Flowchart Export v5.4 (FamilyDescription Parameter Capture)
#
# CHANGELOG from v5.3:
...
in4              = IN[4] if len(IN) > 4 else None   # output_params_csv    (optional)
in5              = IN[5] if len(IN) > 5 else None   # output_types_csv     (optional)
...
def write_debug_txt(path_txt, out_dict, log_lines):
    try:
        ...
        with open(path_txt, "w", encoding="utf-8") as f:
            f.write("=== Flowchart Export v5.4 Debug Output ===\n\n")
            f.write("OUT (json):\n")
            try:
...
def write_sharepoint_list_exports(
    ...
    try:
        ...
        for fam_name in sorted_names_local:
            ...
            # [v5.4-1] FamilyDescription shared parameter value (empty if absent)
            family_desc = sanitize_csv_value(family_descriptions.get(fam_name, ""))

            catalog_rows.append([
                sanitize_csv_value(fam_name), key, kind, system_value, ts,
                parameter_count, has_formulas, has_slots, "Active", family_desc
            ])

        with open(path_catalog, "w", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            # [6] Added HasSlotInstances column; [v5.4-1] Added FamilyDescription column
...
sp = write_sharepoint_list_exports(
    output_catalog_csv, output_edges_csv, output_params_csv, output_types_csv,
...
OUT = {
    ...
    "output_catalog_csv": output_catalog_csv,
    "output_edges_csv": output_edges_csv,
    "output_params_csv": output_params_csv,
    "output_types_csv": output_types_csv,
