...
# Flowchart Export v5.5 (JSON Export for SharePoint Lists)
#
# CHANGELOG from v5.4:
# [v5.5-1] SharePoint exports (FamilyCatalog, FamilyEdges, FamilyParameters, FamilyTypes)
#           now written as JSON arrays instead of CSV. Eliminates comma/quoting issues
#           that were corrupting Formula values in FamilyParameters on upload to Lists.
# [v5.5-2] JSON field names revised: first field in every record is now "Title" to
#           match the SharePoint Lists primary column requirement.
#           FamilyCatalog  "Title" = FamilyName (was "FamilyName" column)
#           FamilyEdges    "Title" = EdgeKey    (was "EdgeKey" column)
#           FamilyParameters "Title" = ParamKey (was "ParamKey" column)
#           FamilyTypes    "Title" = TypeKey    (was "TypeKey" column)
# [v5.5-3] Boolean fields (HasFormulas, HasSlotInstances, IsShared, IsInstance) now
#           written as JSON true/false instead of "TRUE"/"FALSE" strings.
#           HasFormulas and ParameterCount are null (not "") when param stats unavailable.
# [v5.5-4] FamilyParameters gains a ParameterDescription field (empty string placeholder)
#           for team-authored parameter descriptions.
# [v5.5-5] IN[2]-IN[5] now resolve to .json paths instead of .csv paths.
#           _resolve_sp_path detects .json extension (folder paths still accepted).
#
# CHANGELOG from v5.3:
# [v5.4-1] FamilyDescription shared parameter now captured from each family document.
#           collect_family_description() reads the Text Type parameter named
#           "FamilyDescription" from FamilyManager, taking the first non-empty value
#           across all types (falls back to CurrentType if types list is empty).
# [v5.4-2] FamilyCatalog CSV gains a FamilyDescription column (last column).
#           Empty string written for any family where the parameter is absent or blank.
#           Value is sanitized with sanitize_csv_value() before writing.
#
# CHANGELOG from v5.2:
...
#
# Exports:
# - Main flowchart CSV:           <out_folder>/<host>.csv             (auto from IN[0], unchanged)
...
#     FamilyCatalog JSON:         IN[2] explicit  OR  auto: <out_folder>/<host>__FamilyCatalog.json
#     FamilyEdges JSON:           IN[3] explicit  OR  auto: <out_folder>/<host>__FamilyEdges.json
#     FamilyParameters JSON:      IN[4] explicit  OR  auto: <out_folder>/<host>__FamilyParameters.json
#     FamilyTypes JSON:           IN[5] explicit  OR  auto: <out_folder>/<host>__FamilyTypes.json
#
# FamilyCatalog JSON fields:  Title, FamilyKey, Kind, System, LastExported,
#                             ParameterCount, HasFormulas, HasSlotInstances, Status, FamilyDescription
# FamilyEdges JSON fields:    Title, ParentKey, ChildKey, EdgeType, LastExported
# FamilyParameters JSON fields: Title, FamilyKey, ParamName, DataType, DataTypeIdRaw,
#                               Group, Formula, IsShared, IsInstance, ParamGuid, ParameterDescription
# FamilyTypes JSON fields:    Title, FamilyKey, TypeName, LastExported
#
# Inputs:
...
# IN[2] = output_catalog_json  (string, optional)  FamilyCatalog JSON destination
# IN[3] = output_edges_json    (string, optional)  FamilyEdges JSON destination
# IN[4] = output_params_json   (string, optional)  FamilyParameters JSON destination
# IN[5] = output_types_json    (string, optional)  FamilyTypes JSON destination
#           Each accepts three forms:
#             None / empty      -> auto: <out_folder>/<host>__<Suffix>.json
#             folder path       -> join: <folder>/<host>__<Suffix>.json
#             full .json path   -> used as-is
...
in2              = IN[2] if len(IN) > 2 else None   # output_catalog_json  (optional)
in3              = IN[3] if len(IN) > 3 else None   # output_edges_json    (optional)
in4              = IN[4] if len(IN) > 4 else None   # output_params_json   (optional)
in5              = IN[5] if len(IN) > 5 else None   # output_types_json    (optional)
template_graphml = IN[6] if len(IN) > 6 else None   # template_graphml     (required)
include_profiles = bool(IN[7]) if len(IN) > 7 else True
debug            = bool(IN[8]) if len(IN) > 8 else False
...
# Accepts three forms:
#   None / empty       -> auto:  <out_folder>/<host>__<suffix>.json
#   folder path        -> join:  <in_val>/<host>__<suffix>.json
#   full file path     -> as-is: <in_val>
def _resolve_sp_path(in_val, default_filename):
    """Resolve a SharePoint JSON output path.

    in_val may be None/empty (auto-derive into out_folder),
    a directory path (append default_filename inside it),
    or a full file path ending in .json (use as-is).
    """
    if not in_val or not isinstance(in_val, str) or not in_val.strip():
        return os.path.join(out_folder, default_filename)
    val = in_val.strip()
    # If the value doesn't end in .json, treat it as a folder
    if not val.lower().endswith(".json"):
        return os.path.join(val, default_filename)
    return val

output_catalog_json = _resolve_sp_path(in2, host_name + "__FamilyCatalog.json")
output_edges_json   = _resolve_sp_path(in3, host_name + "__FamilyEdges.json")
output_params_json  = _resolve_sp_path(in4, host_name + "__FamilyParameters.json")
output_types_json   = _resolve_sp_path(in5, host_name + "__FamilyTypes.json")
...
try:
    if os.path.abspath(output_graphml).lower() == os.path.abspath(template_graphml).lower():
        raise Exception(
            "Refusing to overwrite template_graphml. Set IN[1] to a different path than IN[6]."
        )
except Exception:
...
def write_debug_txt(path_txt, out_dict, log_lines):
    try:
        ...
        with open(path_txt, "w", encoding="utf-8") as f:
            f.write("=== Flowchart Export v5.5 Debug Output ===\n\n")
            f.write("OUT (json):\n")
            try:
...
# [v5.4-1] FamilyDescription values captured during scan_doc
family_descriptions = {}             # fam_name -> FamilyDescription string
...
# =============================================================================
# [v5.4-1] FamilyDescription capture
# Reads the shared Text Type parameter "FamilyDescription" from the family
# document's FamilyManager.  Because it is a Type parameter, the value is
# retrieved per-type; we take the first non-empty value found.  Falls back to
# CurrentType when the types list is exhausted without finding a value.
# Stores result in family_descriptions[fam_name] (empty string if absent).
# =============================================================================
def collect_family_description(fam_doc, fam_name):
    fm = None
    try:
        fm = fam_doc.FamilyManager
    except:
    ...
    # Locate the FamilyDescription FamilyParameter by name
    desc_fp = None
    try:
        for fp in list(fm.Parameters):
            try:
                if safe_str(fp.Definition.Name).strip() == "FamilyDescription":
                    desc_fp = fp
                    break
            except:
                continue
    ...
    if desc_fp is None:
        return  # Parameter not present in this family; leave entry absent

    # Read from the first type that carries a non-empty value
    try:
        for ft in list(fm.Types):
            try:
                val = ft.AsString(desc_fp)
                if val and val.strip():
                    family_descriptions[fam_name] = val.strip()
                    return
            except:
                continue
    except:
    ...
    # Fallback: CurrentType (handles families with no named types)
    try:
        ct = fm.CurrentType
        if ct is not None:
            val = ct.AsString(desc_fp)
            if val and val.strip():
                family_descriptions[fam_name] = val.strip()
    except:
        pass
...
def scan_doc(fam_doc, depth):
    ...
    collect_family_description(fam_doc, current)     # [v5.4-1]
...
# =============================================================================
# SharePoint Lists export — JSON
# [v5.5] All four lists now written as JSON arrays of objects (not CSV).
#        Eliminates comma/quoting issues in Formula values.
#        First key in every record is "Title" to match SharePoint primary column.
# [v5.3] Each output file path is passed in explicitly (resolved above from IN[]).
# [3]    FamilyEdges exports ALL observed edge types per pair.
# [8]    Per-file error handling: failures do not cascade.
# [10]   Per-family scan timestamps where available.
# =============================================================================
def write_sharepoint_list_exports(
        path_catalog, path_edges, path_params, path_types,
        adj_dict, edge_types_dict, kind_resolver):
    ...
    try:
        ...
        for fam_name in sorted_names_local:
            ...
            # ParameterCount and HasFormulas are null when stats not yet captured
            stats = family_param_stats.get(fam_name, None)
            if stats is None:
                parameter_count = None
                has_formulas = None
            else:
                parameter_count = stats.get("count", None)
                has_formulas = bool(stats.get("has_formulas", False))

            # [10] Per-family scan timestamp if available, else batch timestamp
            ts = family_scan_timestamps.get(fam_name, batch_ts)

            # [6] Slot-instance flag as JSON boolean
            has_slots = bool(slot_instance_children.get(fam_name))

            # [v5.4-1] FamilyDescription shared parameter value
            family_desc = family_descriptions.get(fam_name, "")
            ...
            catalog_records.append({
                ...
                "HasSlotInstances": has_slots,
                "Status":           "Active",
                "FamilyDescription": family_desc
            })

        with open(path_catalog, "w", encoding="utf-8") as f:
            json.dump(catalog_records, f, indent=2, ensure_ascii=False)

        results["familycatalog_json"] = path_catalog
        results["familycatalog_rows"] = len(catalog_records)
    ...
    # --- FamilyEdges ---
    # [3] Export ALL observed edge types per pair using edge_all_types.
    # Title (EdgeKey) includes type suffix for SharePoint uniqueness.
    # e.g., pk->ck::Instance and pk->ck::Repair can coexist as separate records.
    try:
        ...
        with open(path_edges, "w", encoding="utf-8") as f:
            json.dump(edge_records, f, indent=2, ensure_ascii=False)

        results["familyedges_json"] = path_edges
        results["familyedges_rows"] = len(edge_records)
    ...
    # --- FamilyParameters ---
    # [v5.5] Formula written as a plain JSON string — no CSV quoting/escaping issues.
    # [v5.5] IsShared and IsInstance stored as JSON true/false booleans.
    # [v5.5-4] ParameterDescription placeholder field added (empty for now).
    try:
        ...
        for r in sorted(family_params_rows, key=lambda x: (x["FamilyKey"], x["ParamName"])):
            param_records.append({
                ...
                "DataType":             r["DataType"],
                "DataTypeIdRaw":        r.get("DataTypeIdRaw", ""),
                "Group":                r["Group"],
                "Formula":              r["Formula"],
                "IsShared":             r["IsShared"] == "TRUE",
                "IsInstance":           r["IsInstance"] == "TRUE",
                "ParamGuid":            r["ParamGuid"],
                "ParameterDescription": ""
            })

        with open(path_params, "w", encoding="utf-8") as f:
            json.dump(param_records, f, indent=2, ensure_ascii=False)

        results["familyparameters_json"] = path_params
        results["familyparameters_rows"] = len(param_records)
    ...
    # --- FamilyTypes ---
    # One record per (FamilyKey, TypeName) pair.
    try:
        ...
        for fam_name in sorted(family_types_by_name.keys()):
            ...
            for tn in sorted(family_types_by_name[fam_name]):
                ...
                type_records.append({
                    "Title":        type_key,
                    "FamilyKey":    fam_key,
                    "TypeName":     tn_stripped,
                    "LastExported": ts
                })

        with open(path_types, "w", encoding="utf-8") as f:
            json.dump(type_records, f, indent=2, ensure_ascii=False)

        results["familytypes_json"] = path_types
        results["familytypes_rows"] = len(type_records)
...
sp = write_sharepoint_list_exports(
    output_catalog_json, output_edges_json, output_params_json, output_types_json,
    adj, edge_types, determine_function)
...
OUT = {
    ...
    "output_catalog_json": output_catalog_json,
    "output_edges_json": output_edges_json,
    "output_params_json": output_params_json,
    ...
    "families_with_description": len(family_descriptions),   # [v5.4-1]
